...
public class SortingAndCountingUtils {
  ...
  public static List uniqCountListOfArrays(List list, final int fieldIndex) {
	...
	/**
	 * Compute the Hirsch index (h-index) for an list of citation counts.
   ...
   * 
   * @param citationCounts  not-necessarily-sorted list of citation counts: THIS WILL BE REVERSE SORTED
   * @return
	 */
	public static <T extends Comparable<Integer>> Integer hirschIndex(List<T> citationCounts) {
	  if (citationCounts.size() == 0) { return 0; }
	  Collections.sort(citationCounts, Collections.reverseOrder());
	  for (Integer i = 1; i <= citationCounts.size(); i++) {
	    T count = citationCounts.get(i-1);
	    if (count.compareTo(i) < 0) { return i-1; }
	  }
