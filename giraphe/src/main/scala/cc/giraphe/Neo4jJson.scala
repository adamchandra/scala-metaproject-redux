...
trait Neo4jJson {
  ...
  def json(seq: (String, Any)*): JSONObject = {
    val obj = new JSONObject
    for ((key, value) <- seq) 
      obj.put(key, value.asInstanceOf[AnyRef])
    obj
  }
  ...
  implicit def jsonConversion(json: JSONObject)(implicit neo: GraphDatabaseService): Node = 
